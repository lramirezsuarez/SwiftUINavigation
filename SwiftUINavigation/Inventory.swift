...
class InventoryViewModel: ObservableObject {
    ...
    private func bind(itemRowViewModel: ItemRowViewModel) {
        ...
        itemRowViewModel.$route.map { [id = itemRowViewModel.id] route in
            route.map { Route.row(id: id, route: $0)}
        }
        .removeDuplicates()
        .dropFirst()
        .assign(to: &self.$route)
        self.$route.map { [id = itemRowViewModel.id] route in
...
struct InventoryView: View {
    ...
    var body: some View {
        List {
            ForEach(self.viewModel.inventory, content: ItemRowView.init(viewModel:))
        }
        .toolbar {
        ...
        .sheet(item: self.$viewModel.route.case(/InventoryViewModel.Route.add)
                                               ) { itemToAdd in
            NavigationView {
        ...
        NavigationView {
            InventoryView(
                viewModel: .init(
                    ...
                    route: nil//.init(name: "", color: nil, status: .inStock(quantity: 1))
                )
            )
        }
    }
